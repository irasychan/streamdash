...
1. **Server-side platform connections** - All external connections (Twitch IRC, Discord Gateway, YouTube polling) happen on the server. This handles CORS, keeps tokens secure, and allows message normalization before broadcasting.

1. **Unified message format** - All platform messages normalized to a common `ChatMessage` type before being sent to clients.

1. **ConnectionManager singleton** - Central manager handles all platform connections and broadcasts to multiple SSE clients.

1. **Admin panel layout** - Chose shadcn sidebar component for consistent, accessible navigation with built-in mobile support and keyboard shortcuts.
...
1. **Client-side Preferences (localStorage)**
...
1. **Added platform validation in chat connect**
