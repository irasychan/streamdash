...
1. **Zustand for state management** - Chose Zustand over context providers for reduced boilerplate, better performance, and simpler mental model. Store is split into slices for organization.

1. **Services layer separation** - External API clients (TwitchIRC, YouTubePoller, DiscordBridge) moved to `services/` directory, keeping them free of React dependencies.

1. **Server-only config** - Configuration persistence logic moved to `server/` directory to clearly indicate server-only code that should not be imported by client components.
