...
export async function POST(request: Request) {
  try {
    ...
    if (!targetUserId) {
      return NextResponse.json({ ok: false, error: "Missing target user" }, { status: 400 });
    }
    ...
    if (!authToken.accessToken) {
      return NextResponse.json({ ok: false, error: "Twitch OAuth required" }, { status: 401 });
    }
    ...
    if (!channelLogin) {
      return NextResponse.json({ ok: false, error: "Missing Twitch channel" }, { status: 400 });
    }
    ...
    if (!broadcasterId) {
      return NextResponse.json({ ok: false, error: "Invalid Twitch channel" }, { status: 400 });
    }
    ...
    return NextResponse.json({ ok: false, error: "Moderation request failed" }, { status: 500 });
  }
}
