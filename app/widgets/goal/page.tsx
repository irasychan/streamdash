...
function GoalWidgetContent() {
  ...
  const goalPercent = Math.min(100, Math.round((stats.goal.current / stats.goal.target) * 100));
  ...
  return (
    ...
    >
      <CardContent className="pb-4 pt-5">
        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Follower sprint</p>
        <p className="mt-2 font-display text-4xl text-foreground">{goalPercent}%</p>
        ...
        <p className="mt-2.5 text-sm text-muted-foreground">Keep pushing to the next milestone.</p>
      </CardContent>
    </Card>
  );
}
