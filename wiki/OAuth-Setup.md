...
1. Add redirect URL: `http://localhost:3000/api/twitch/callback` (add your production URL too).
1. Copy **Client ID** and **Client Secret** into `.env.local`:
   ```bash
   TWITCH_CLIENT_ID=
   TWITCH_CLIENT_SECRET=
   ```
1. Start the auth flow: visit `http://localhost:3000/api/twitch/auth`.
...
1. Add redirect URL: `http://localhost:3000/api/discord/callback` (add your production URL too).
1. Copy credentials into `.env.local`:
   ```bash
   DISCORD_CLIENT_ID=
   DISCORD_CLIENT_SECRET=
   ```
1. Start auth: visit `http://localhost:3000/api/discord/auth`.
...
1. Copy the bot token:
   ...
   DISCORD_BOT_TOKEN=
   ```
1. Go to **OAuth2 > URL Generator**, select `bot` scope with permissions:
